...
import numpy as np
#import skin_disease_detection as sdd
...
def Smain():
    ...
    with st.sidebar:
            st.image('Skin_Disease_detection_main/mg.png')
            st.title("Dermatrix")
    ...
    @st.cache_resource()
    def load_model():
        model=tf.keras.models.load_model('Skin_Disease_detection_main/G_skin.h5')
        return model
    ...
    if file is None:
        ...
        #loss, acc = model.evaluate(sdd.test_gen)
        #st.sidebar.info("Accuracy : " + str(x) + " %")
        acc=np.load('Skin_Disease_detection_main/acc.npy')
        st.sidebar.info("Accuracy : " + str(acc) + " %")
